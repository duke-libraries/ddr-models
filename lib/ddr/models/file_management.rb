  ...
  module Models
    module FileManagement
      ...
      def add_external_file file, dsid, opts={}
        ...
        raise Ddr::Error, "Cannot add external file to datastream when dsLocation change is pending." if ds.dsLocation_changed?
        ...
        if opts[:use_original]
          raise Ddr::Error, "Cannot add file to repository that is owned by another user." unless File.owned?(file_path)
          store_path = file_path
        else
          # generate storage path
          file_name = opts[:file_name] || Ddr::Utils.file_name_for(file)
      ...
      def virus_scan
        if Ddr::Utils.file_or_path?(file_to_add) # can't virus scan blob
          virus_scan_results << Ddr::Services::Antivirus.scan(file_to_add) 
        end
      end
      ...
      def generate_external_directory_subpath 
        ...
        m = Ddr.external_file_subpath_regexp.match(dirname)
