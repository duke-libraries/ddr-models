...
module Ddr
  module Services
    class Antivirus
      ...
      class << self
        def scan(file)
          result = scan_one(Ddr::Utils.file_path(file)) # raises ArgumentError
          raise Ddr::Models::VirusFoundError, result if result.has_virus?
          raise Ddr::Models::Error, "Antivirus error (#{result.version})" if result.error?
      ...
      class ScanResult
        ...
        def initialize(raw, file_path, opts={})
          ...
          @version = Ddr::Services::Antivirus.version
        end
