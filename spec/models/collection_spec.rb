...
RSpec.describe Collection, type: :model do
  ...
  describe "roles granted to creator" do
    ...
    it "should include Curator roles in both resource abd policy scopes" do
      expect(subject.roles.to_a).to eq([Ddr::Auth::Role.new(role_type: "Curator", agent: user.agent, scope: "resource"), Ddr::Auth::Role.new(role_type: "Curator", agent: user.agent, scope: "policy")])
    end
  end
